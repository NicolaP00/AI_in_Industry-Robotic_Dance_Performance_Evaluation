...
def Anova_Decomposition(model, X, y, preprocessor, lf):
   ...
   # SECOND PART - Oneway interactions 
    for i in range(X_preprocessed.shape[1]):
        ...
        model.fit(X_preprocessed[:,i].reshape(-1,1), y) 
        Predicted_labels = model.predict(X_preprocessed[:,i].reshape(-1,1))
    ...
    for i in range(X_preprocessed.shape[1]):
        ...
        for j in range(X_preprocessed.shape[1]):
            if i!=j:
               model.fit(X_preprocessed[:,[i, j]], y)
               Predicted_labels = model.predict(X_preprocessed[:,[i,j]])
